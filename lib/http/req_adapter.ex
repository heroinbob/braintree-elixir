...
if is_compiled and is_included do
  defmodule Braintree.HTTP.ReqAdapter do
    ...
    * `:public_key` - Override the public used for authentication.
    ...
    Responses are generally handled the same as Hackney to provide a seamless switch out.

    Errors are handled differently. All exceptions that are caught are always returned as
    a `Braintree.HTTP.Error` exception. This way your code can decide what to do with it.
    The goal is that there is never a dependency exception raised.
    """
